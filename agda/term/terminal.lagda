...
record _≈_ {F : Container _ _}(a b : ν F) : Set where
    ...
    field
      ...
      outsnd : ∀ x → tail a x ≈ tail b (subst (Container.Position F) outfst x)
open _≈_ public
...
univ-from : {F : Container _ _}{C : Set}(h : C → ν F){c : C → ⟦ F ⟧ C} →
                            out ∘ h ≡ map h ∘ c → (x : C) → A⟦ c ⟧ x ≈ h x
outfst (univ-from h eq x) = ,-injectiveˡ (cong-app (sym eq) x)
outsnd (univ-from {F} h {c} eq x) y = let expr = tail (h x) (subst (Position F) (,-injectiveˡ (cong-app (sym eq) x)) y) in
  univ-from
    (const (tail (h x) (subst (Position F) (outfst (univ-from h eq x)) y)))
    (begin
      out ∘ const expr
      ≡⟨ funext (λ z → Σ-≡,≡→≡ ({!!} , {!!})) ⟩
      map (const expr) ∘ c
    ...
    (c x .snd y)
    --(begin
    --  out ∘ const (tail (h x) y)
    --  ≡⟨ funext (λ z → Σ-≡,≡→≡ ({!!} , {!!})) ⟩
    --  map (const (tail (h x) y)) ∘ c
    --∎)
...
outsnd (reflection x) y = reflection (out x .snd y)
\end{code}
