...
b a (x, y) = loop x
  where loop x = if x > y
    ...
    \end{spec}
    This way, less data needs to be pushed around for each (recursive) function call.
    \item Ensure that functions are inlined properly. So for the second example above add a pragma that inlines the function.
    This maximises the time that haskell can apply other optimizations between the bodies of multiple fusing functions:
    \begin{spec}
        {-# INLINE betweenCh #-}
    \end{spec}
...
su h s = loopt s 0
  where loopt s' sum = case h s' of
    ...
    \end{spec}
    For Church encodings, it is a little bit more tricky to get the resultant function to be tail-recursive, it is possible however. Taking the algebra for sum again:
    \begin{spec}
sum2 :: List' Int -> Int
sum2 = sumCh . toCh
...
I measured the performance using tastybench\footnote{\url{https://hackage.haskell.org/package/tasty-bench}}
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.7\textwidth]{figures/Execution time for all pipelines.svg}
    \caption{Temp1}
    \label{fig:results1}
\end{figure}
\begin{figure}[H]
    \centering
    \includesvg[width=0.7\textwidth]{figures/Execution time for all pipelines - stacked.svg}
    \caption{Temp1}
    \label{fig:results2}
\end{figure}
